...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from):
    ...
    gaussians = GaussianModel(0,0.003)
...
def my_CF_gaus(dataset,opt,pipe,debug_from,iterations,const_scale,crit_vgg,crit_tv):
    ...
    for i in range(1,image_cnt+1):
        ...
        if i == 1:
            file_path = os.path.join(dataset.model_path, "point_cloud/iteration_30000/point_cloud.ply")
        else :
        ...
        scene = Scene(dataset, gaussians,regist=False)
        ...
        for iteration in train_bar:
            ...
            # Loss
            # L1 loss
            gt_filename = f"{(i) :04d}.jpg"
            tem_path = os.path.join(os.path.dirname(dataset.model_path), "images")
            ...
            LAMBDA_DSSIM = 0.0
            loss = (1.0 - LAMBDA_DSSIM) * Ll1 + LAMBDA_DSSIM * (1.0 - ssim(image, resized_gt_image))
            ...
            with torch.no_grad():
                ...
                if iteration < 2 or iteration % 100 == 0 :
                    image = image.detach().cpu().permute(1, 2, 0).numpy() * 255
                    image = image.astype('uint8')
                    image_bgr = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                    img_list = [
                        # image.detach().cpu().permute(1, 2, 0).numpy() * 255,
                        image_bgr,
                        resized_gt_image.detach().cpu().permute(1, 2, 0).numpy() * 255
                    ]
                    ...
                    cv2.imwrite(os.path.join(img_save_path, f"{iteration}.jpg"), img_list)
